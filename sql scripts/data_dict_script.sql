...
metadata AS (
  SELECT
    ...
    CONCAT(
      ...
      CASE 
        WHEN tut.data_type IN('varchar','char') THEN CONCAT('(', CAST(tut.CHARACTER_MAXIMUM_LENGTH AS varchar(10)), ')')
        ...
        WHEN tut.NUMERIC_PRECISION IS NOT NULL AND tut.NUMERIC_SCALE IS NULL THEN CONCAT('(', CAST(tut.NUMERIC_PRECISION AS varchar(10)), ')')
        WHEN tut.NUMERIC_PRECISION IS NOT NULL AND tut.NUMERIC_SCALE IS NOT NULL THEN CONCAT('(', CAST(tut.NUMERIC_PRECISION AS varchar(10)), ',', CAST(tut.NUMERIC_SCALE AS varchar(10)), ')')
        ELSE ''
      END
    ) AS data_typ,
    CASE 
  ...
  INNER JOIN baseTbl bt 
    ON bt.table_catalog = tut.TABLE_CATALOG AND bt.table_name = tut.table_name
),
meta_for_keys AS (
  SELECT 
    schema_nm, 
...
SELECT 
  ...
  md.OBJ_TYP,
  md.ORD_POS AS ord,
  COALESCE(mfk.is_key, ' ') AS is_key,
  md.COLUMN_NM, 
  md.DATA_TYP, 
  md.NULLABLE, 
  c.column_descr 
FROM metadata md
LEFT JOIN meta_for_keys mfk
  ON md.SCHEMA_NM = mfk.SCHEMA_NM
  AND md.TABLE_NM = mfk.TABLE_NM
  AND md.COLUMN_NM = mfk.COLUMN_NM
LEFT JOIN col_comm c
  ON md.SCHEMA_NM = c.SCHEMA_NM
  AND md.TABLE_NM = c.TABLE_NM
  AND md.COLUMN_NM = c.COLUMN_NM;
